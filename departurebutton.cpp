...
DButton::DButton(int x1,int y1,int height1, int width1){
    x= x1;y=y1;height=height1; width = width1;
}



void DButton::drawFrame ( ) {
    glColor3ub(0,0,0);
    glLineWidth(3.0);
    ...
    YsGlDrawFontBitmap6x10("Departure Time");
}

void DButton::drawBox (double ybox) {
...
int DButton::drawText (double ybox){
    glColor3ub(0, 0, 255);
    int baseT;
    if (y-ybox < height/8) {
...
int DButton::returntime(int baseT, int addT){
    int Dtime = baseT + addT;
    return Dtime;
}
